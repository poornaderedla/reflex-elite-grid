...
const RotationGame: React.FC<RotationGameProps> = ({ onFinish }) => {
  ...
  const [shapes, setShapes] = useState<Array<{ id: number; shape: string; rotation: number; isTarget: boolean }>>([]);
  ...
  const handleShapeClick = (clickedShape: { id: number; shape: string; rotation: number; isTarget: boolean }) => {
    ...
    if (clickedShape.isTarget) {
      ...
      setFeedback("Wrong! Find the differently rotated shape");
    }
        ...
        <div className="w-full max-w-md bg-luxury-black rounded-2xl shadow-2xl border-2 border-luxury-gold p-6 flex flex-col items-center animate-fade-in">
          ...
          <div className="w-full mb-4 p-3 rounded-lg bg-luxury-gold/10 border border-luxury-gold/40 text-center">
            ...
            <div className="text-sm text-luxury-white/80 mb-1">
              Find the shape with different rotation as quickly as possible. Spatial awareness matters!
            </div>
          ...
          <div className="text-center mb-4">
            ...
            <div className="text-lg text-white mb-1">Rotation Results</div>
          </div>
          ...
          <div className="flex gap-4 mt-4 w-full">
            ...
            <button
              ...
              onClick={() => navigator.share && navigator.share({ title: 'Rotation Results', text: `Score: ${score}, Correct: ${correctAnswers}, Avg: ${stats?.average ?? 0}ms`, url: window.location.href })}
            >
              Share
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] p-4 space-y-4">
      ...
      <div className="text-center space-y-2">
        <div className="text-2xl font-bold text-luxury-gold">Find the different rotation</div>
        <div className="text-sm text-luxury-white/70">One shape has a different rotation than the others</div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 w-80">
        {shapes.map((shape) => (
          <button
            ...
            className="w-20 h-20 bg-luxury-black border-2 border-luxury-white/20 rounded-lg text-3xl font-bold text-luxury-gold hover:border-luxury-gold/50 hover:bg-luxury-gold/10 transition-all hover:scale-105 active:scale-95 flex items-center justify-center"
      ...
      <div className="text-center text-xs text-luxury-white/50">
        Click the shape with different rotation
      </div>
    </div>
  );
};

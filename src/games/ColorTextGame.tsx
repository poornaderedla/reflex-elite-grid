...
const ColorTextGame: React.FC<ColorTextProps> = ({ onFinish }) => {
  ...
  const calculateStats = () => {
    ...
    let average = 0;
    if (times.length > 5) {
      // Drop the slowest round
      times = times.filter(t => t !== Math.max(...times));
  ...
  if (showResults && gameEndTime) {
    ...
    return (
      <div className="w-full min-h-[calc(100vh-10rem)] flex items-center justify-center bg-luxury-black p-6">
        <div className="relative flex flex-col w-full max-w-md min-h-[60vh] bg-luxury-black rounded-2xl shadow-2xl border-2 border-luxury-gold animate-fade-in mx-auto">
          {/* Scrollable content */}
          <div
            ref={contentRef}
            className="px-6 pt-8 pb-8 flex flex-col items-center w-full"
          >
          ...
          <div className="flex gap-4 mt-4 w-full px-6 pb-6">
            <button
